...
// unified byte alias used across oodle code
#ifndef LEGILIMENS_BYTE_DEFINED
  #define LEGILIMENS_BYTE_DEFINED
...
inline int printf_error(const char* format, ...) {
	va_list args;
	va_start(args, format);
	std::fputs("[31m[Error] ", stdout);
...
inline int printf_verbose(bool enabled, const char* format, ...) {
	if (!enabled) return 0;
	va_list args;
	va_start(args, format);
	std::fputs("[34m[Info] ", stdout);
	int result = std::vprintf(format, args);
	std::fputs("[0m", stdout);
	va_end(args);
	return result;
